...
**Staging and crash recovery follow the plugin vocabulary.** `.tmp-<pid>-<nanos>` → `.new-<…>` → `.backup-<…>` with `.dsh-id` markers, grouped by id on `reconcile()`. A startup `reconcile()` also re-links broken store→root entries, removes lingering entries for disabled skills, and sweeps active-root symlinks that point into the store but match no current inventory row. Plain files, directories, and links pointing outside `skills-store/` are user content and never touched.

**Community catalog reuses the plugin hub feed shape.** Single URL `https://dsh-plugin.org/api/skills.zh.json` parsed as either hub short-key array or `{items:[...]}`; unreachable or un-deployed sources degrade to an empty catalog with UI guidance, not an error. Cache lives at `<data_dir>/skills-catalog.json` with a 6-hour TTL — same freshness window as the plugin center.
...
- The community catalog URL is a placeholder until the hub ships a skills feed; the cache directory and TTL machinery exist regardless, so the feature lights up the moment a feed is published.
