...
- **官方对话快捷入口**：管理面板「概览」页的「打开官方对话」按钮拉起独立的 `official-chat` 窗口（label `official-chat`），固定加载 DeepSeek 官方对话页 `https://chat.deepseek.com`，由外壳注入 chrome-row 顶部条带 + 拉绳挂件。窗口创建时不再覆盖 user-agent——WebView2 引擎本身就是真实的桌面版 Edge，原生 UA、`Sec-CH-UA` 客户端提示与 `navigator.userAgentData` 天然一致（此前改写成 Chrome 反而制造跨层矛盾，正是环境检测的特征）；并附 `.incognito(true)`，让 chat.deepseek.com 的环境检查把它当成普通桌面浏览器处理；同一按钮在窗口已开时变为「关闭官方对话」，复用现有窗口并 `set_focus`，OS 关闭按钮也保持有效（该窗口不持有内核会话）。创建窗口时还经 `.additional_browser_args(OFFICIAL_CHAT_BROWSER_ARGS)` 追加 Chromium 启动开关（wry 默认 `msWebOOUI` / `msPdfOOUI` / `msSmartScreenProtection` 之上叠加 `AutomationControlled`、`TranslateUI`、`InterestFeedContentSuggestions` 与 `--disable-blink-features=AutomationControlled`）：既抑制 `navigator.webdriver = true`，也不弹 SmartScreen 安全提醒与 Edge 专属 UI；该参数仅 WebView2 后端消费，macOS / Linux 构建自动忽略；同时窗口固定使用专属 user-data 目录 `<data_dir>/webview-official-chat`（WebView2 要求同一目录上的环境参数一致，不隔离会导致窗口创建失败）；之后注入三个 `initialization_script`，顺序固定为 `pullstring-launcher.js` → `titlebar-pulse.js` → `chat-fingerprint.js`：第一条先把真实 `window.__TAURI__` 闭包捕获到 `__DSH_TAURI_REF__`，最后一条把 `__TAURI_*` 嵌入式全局彻底删除并把 `navigator.webdriver` 钉在 `false`，拉绳挂件凭先前捕获的引用仍能正常拉起管理面板。
...
- 「打开官方对话」：管理面板「概览」页一键拉起独立的官方对话窗口，加载 `https://chat.deepseek.com`，使用原生 Edge UA、incognito 会话、专属 user-data 目录与 `OFFICIAL_CHAT_BROWSER_ARGS` 浏览器开关（隐藏 `navigator.webdriver`、关闭 SmartScreen 提醒与 Edge 专属 UI）让 chat.deepseek.com 的环境检查把它视为普通浏览器；注入 `pullstring-launcher.js` → `titlebar-pulse.js` → `chat-fingerprint.js` 三个初始化脚本，依次完成「捕获真实 `window.__TAURI__` 到闭包」「挂 chrome-row 顶部条带」「钉住 `navigator.webdriver = false` 并删除 `__TAURI_*` 嵌入式全局」（重复点击复用现有窗口并聚焦；窗口已开时按钮文案翻为「关闭官方对话」并触发 `close_official_chat`，OS 关闭按钮正常工作）
...
6. 「打开官方对话」：在「概览」页点击此按钮即可拉起独立的 DeepSeek 官方对话窗口（chrome-row 顶部条带为 DeepSeek 官方蓝 `#4D6BFE`、拉绳挂件贴左边距 12px；区别于工作台窗口的 Gitea 绿色 212px 偏移），与工作台窗口互不干扰；窗口创建时不覆盖 user-agent（诚实呈现桌面版 Edge）、附 incognito 会话，并经 `.additional_browser_args(OFFICIAL_CHAT_BROWSER_ARGS)` 注入 Chromium 开关（含 wry 默认三项与 `AutomationControlled` 等：不自报 `navigator.webdriver = true`、不弹 SmartScreen 安全提醒），随后注入三个 `initialization_script` 完成浏览器指纹伪造（`pullstring-launcher.js` 捕获真实 `__TAURI__` 引用 → `titlebar-pulse.js` 挂顶部条带 → `chat-fingerprint.js` 伪造 `navigator.userAgentData` / `window.chrome` / `navigator.webdriver` 并把 `window.__TAURI__` 替换为 neutered Proxy，所以拉绳挂件在 Proxy 生效后仍能拉起管理面板）；窗口已开时按钮变为「关闭官方对话」并销毁当前窗口，OS 关闭按钮始终保持有效。
